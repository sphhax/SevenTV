...
window.addEventListener('7TV#BackgroundExtMessage', (event) => {
	...
	if (ev.detail.tag === 'LoadChannel') {
		// Handle LoadChannel
		// Unload all non-global emotes from the current set
		Page.EmoteSet.filter((emote) => !emote.global).map((emote) => Page.EmoteSet.splice(Page.EmoteSet.indexOf(emote), 1));

		// Append new channel emote
		Page.EmoteSet.push(...ev.detail.emotes);
		Page.ChatListener.sendSystemMessage(
			`Loaded ${ev.detail.emotes.length} emotes in $currentChannel: ${ev.detail.emotes
				.map((e: DataStructure.Emote) => e.name)
				.join(', ')}`
		);
		...
		Page.ChatListener.sendSystemMessage(
			`7TV is outdated! Your version is ${ev.detail.clientVersion}, while the latest is ${ev.detail.latestVersion}`
		);
	}
});
